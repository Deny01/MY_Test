namespace MakeSoft.Tools.BarCodes
{
    ...
    public class BarCodeUPCA : BarCode
    {
        ...
        public override void DrawCode(Graphics g, Point point)
        {
            if (!this.CheckBarCode())
            {
                base.DrawStringErro(g, point.X, point.Y);
            }
            else
            {
                ...
                PointF tf = new PointF(FunctionsGraphics.ConvertDisplayToInch(point.X), FunctionsGraphics.ConvertDisplayToInch(point.Y));
