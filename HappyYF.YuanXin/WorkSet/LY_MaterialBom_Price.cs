...
{
    public partial class LY_MaterialBom_Price : Form
    {
        ...
        public LY_MaterialBom_Price()
        {
            InitializeComponent();
            this.ly_item_allcostTableAdapter.CommandTimeout = 0;
        }
        ...
        private void ly_inma0010DataGridView_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            string selAllString = "SELECT     parentno, itemno, cast(itemno +':'+itemname as char(30))  as itemname,cast(qty_set as decimal(18,3)) as qty_set,itemprice ,itemmoney,assemblymoney,standardsetmoney,itemprocessingmoney,operation_cost,isnull(itemmoney,0)+isnull(itemprocessingmoney,0)+isnull(assemblymoney,0)+isnull(standardsetmoney,0)+isnull(operation_cost,0) as allmoney from f_BomExtend_price('" + s + "',1) ORDER BY  id_num ";
        ...
        private static void Countmoney(string nowplannum)
        {
            ...
            string updstrclear = " update ly_inma0010  " +
                        "  set mprice=0,pprice=0,assembly_price=0,standardset_price=0,operation_cost=0 "
            ...
            string updstr = " update ly_inma0010  " +
                          "  set mprice=dbo.f_Item_price(wzbh,1),pprice=dbo.f_Item_price_processingCost(wzbh,1),assembly_price=dbo.f_Item_price_assemblyCost(wzbh,1),standardset_price=dbo.f_Item_price_standardsetCost(wzbh,1),operation_cost=dbo.f_Item_price_operationCost(wzbh,1) "
                          + " where  wzbh='" + nowplannum + "'";
        ...
        {
            NewFrm.Show(this);
            this.ly_item_allcostTableAdapter.Fill(this.lYMaterialMange.ly_item_allcost, this .parentNum );
            NewFrm.Hide(this);
        }

        private void toolStripButton10_Click(object sender, EventArgs e)
        {
            ExportDataGridviewTOExcellA.ExportDataGridview(this.ly_item_allcostDataGridView, true);
        }
