...
{
    public partial class LY_MaterialBom_Price_noVat2 : Form
    {
        ...
        public LY_MaterialBom_Price_noVat2()
        {
            InitializeComponent();
            //this.ly_item_allcost_NoVat2TableAdapter.CommandTimeout = 0;
        }
        ...
        private void LY_MaterialBom_Load(object sender, EventArgs e)
        {
            ...
            this.ly_item_allcost_NoVat2TableAdapter.Connection.ConnectionString = SQLDatabase.Connectstring;
            ...
            try
            {
                if (dr.Length > 0)
                {
                    foreach (DataRow d in dr)
                    {
                        ...
                        TNode.Text = d["itemname"].ToString() + d["allmoney"].ToString() + "     (" + d["qty_set"].ToString() + "  " + d["itemprice"].ToString() + "  (" + d["itemmoney"].ToString() + ")+(" + d["itemprocessingmoney"].ToString() + ")+(" + d["assemblymoney"].ToString() + ")+(" + d["standardsetmoney"].ToString() + ")+(" + d["operation_cost"].ToString() + "))";
        ...
        private void ly_inma0010DataGridView_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            string selAllString = @"SELECT     parentno, itemno, cast(itemno +':'+itemname as char(30))  as itemname,cast(qty_set as decimal(18,3)) as 
qty_set,itemprice ,itemmoney,assemblymoney,standardsetmoney,itemprocessingmoney,operation_cost,isnull(itemmoney,0)+isnull(itemprocessingmoney,0)+isnull
(assemblymoney,0)+isnull(standardsetmoney,0)+isnull(operation_cost,0) as allmoney from f_BomExtend_price_novat2('" + s + "',1) ORDER BY  id_num ";
        ...
        private static void Countmoney(string nowplannum)
        {
            ...
            string updstrclear = " update ly_inma0010  " +
                          "  set mprice_novat2=0,pprice_novat=0,assembly_price_novat=0,standardset_price_novat=0,operation_cost_novat=0 "
            ...
            string updstr = " update ly_inma0010  " +
                          "  set mprice_novat2=dbo.f_Item_price_novat2(wzbh,1),pprice_novat=dbo.f_Item_price_processingCost_novat(wzbh,1),assembly_price_novat=dbo.f_Item_price_assemblyCost_novat(wzbh,1),standardset_price_novat=dbo.f_Item_price_standardsetCost_novat(wzbh,1),operation_cost_novat=dbo.f_Item_price_operationCost_novat(wzbh,1) "
                          + " where  wzbh='" + nowplannum + "'";
        ...
        {
            NewFrm.Show(this);
            this.ly_item_allcost_NoVat2TableAdapter.Fill(this.lYMaterialMange.ly_item_allcost_NoVat2, this.parentNum);
