...
{
    public partial class LY_Quality_Control_Purchase : Form
    {
       ...
       // string now_order_id = "";
        ...
        public LY_Quality_Control_Purchase()
        {
            InitializeComponent();
        }
        ...
        private void LY_Machine_Load(object sender, EventArgs e)
        {
            ...
            this.ly_purchase_contract_inspectionsingleTableAdapter.Connection.ConnectionString = SQLDatabase.Connectstring;
        ...
        private void ly_production_order_inspectionDataGridView_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            string nowoperator = dgv.CurrentRow.Cells["质检员8"].Value.ToString();
            ...
            if (!string.IsNullOrEmpty(nowoperator))
            {
                ...
                {
                    MessageBox.Show("请质检员:" + nowoperator + "操作", "注意");
                    return;
                }
            }
            ...
            if ("送检数量8" == dgv.CurrentCell.OwningColumn.Name)
            {
                ...
                if (queryForm.NewValue != "")
                {
                    ...
                    dgv.CurrentRow.Cells["质检员8"].Value = SQLDatabase.nowUserName();
                    dgv.CurrentRow.Cells["实检日期8"].Value = SQLDatabase.GetNowdate();
                    ...
                    if (!CheckInput(dgv))
                    {
                        if (string.IsNullOrEmpty(queryForm.OldValue))
                        {
                            ...
                            dgv.CurrentRow.Cells["质检员8"].Value = DBNull.Value;
                            dgv.CurrentRow.Cells["实检日期8"].Value = DBNull.Value;
                        }
                        else
                        {
                            dgv.CurrentRow.Cells["送检数量8"].Value = queryForm.OldValue;
...
//            }
        ...
        {
            if (null == this.ly_purchase_contract_inspectionDataGridView.CurrentRow) return;
            //string wzbh = this.ly_purchase_contract_inspectionDataGridView.CurrentRow.Cells["物料编号8"].Value.ToString();
            //ly_purchase_contract_inspectionBindingSource.Filter= "物料编号='"+wzbh+"'";
            BaseReportView queryForm = new BaseReportView();
             
            queryForm.Text = "中原精密质检报告";
            queryForm.Printdata = this.lYQualityInspector;
            queryForm.PrintCrystalReport = new LY_ZJ_CG();
            queryForm.ShowDialog();
        }
        ...
        private void ly_purchase_contract_inspectionDataGridView_SelectionChanged(object sender, EventArgs e)
        {
            if (null == ly_purchase_contract_inspection_MainDataGridView.CurrentRow || null == ly_purchase_contract_inspectionDataGridView.CurrentRow)
            { 
            ...
            string wzbh = ly_purchase_contract_inspectionDataGridView.CurrentRow.Cells["物料编号8"].Value.ToString();

            this.ly_purchase_contract_inspectionsingleTableAdapter.Fill(this.lYQualityInspector.ly_purchase_contract_inspectionsingle, InspectionCode,wzbh);


            //this.ly_purchase_contract_inspectionsingleTableAdapter.Fill(this.lYQualityInspector.ly_purchase_contract_inspectionsingle, contract_inspection_codeToolStripTextBox.Text, itemnoToolStripTextBox.Text);
        }
    }
}
