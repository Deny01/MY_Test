...
{
    public partial class LY_Machine_Remake : Form
    {
        ...
        private void ly_production_order_remake_detailDataGridView_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (SQLDatabase.CheckHaveRight(SQLDatabase.NowUserID, "返修外协审批"))
            {
            ...
            if ("True" == dgv.CurrentRow.Cells["审批"].Value.ToString())
            {
                MessageBox.Show("已经审批，无法修改！");return;
            }
            if (!string.IsNullOrEmpty(dgv.CurrentRow.Cells["结账单号"].Value.ToString()))
            {
                MessageBox.Show("已经结账，无法修改！"); return;
            }
            
            if ("外协价" == dgv.CurrentCell.OwningColumn.Name)
            {
                ...
                if (queryForm.NewValue != "")
                {
                    dgv.CurrentRow.Cells["外协价"].Value = queryForm.NewValue;
                }
                else
                {
                    dgv.CurrentRow.Cells["外协价"].Value = DBNull.Value;
                }
                this.ly_production_order_remake_detailDataGridView.EndEdit();
