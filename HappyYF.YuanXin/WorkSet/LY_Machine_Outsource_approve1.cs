...
namespace HappyYF.YuanXin.WorkSet
{
    public partial class LY_Machine_Outsource_approve1 : Form
    {
        ...
        private void LY_Machine_Outsource_Load(object sender, EventArgs e)
        {
            ...
            this.lY_outmachine_contract_main_selBindingSource.Filter = "审核  = 'True'";
            ...
            selAllString = " select distinct a.work_code,case when a.work_code='019' then a.work_code+':公司外协'  else a.work_code+':'+ isnull(b.supplier_name,'')+' '+ isnull(b.supplier_contacts,'') end as work_name from ly_production_order_detail a left join ly_supplier_list b on a.work_code=b.supplier_code where (isnull(a.outorder_flag,0)=1) and a.work_code is not null ";
        ...
        private void lY_outmachine_contract_main_selDataGridView_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if ("批准" == dgv.CurrentCell.OwningColumn.Name)
            {
                ...
                for (int i = 0; i < ly_outmachine_contract_detail_selDataGridView.Rows.Count; i++)
                {
                    if (ly_outmachine_contract_detail_selDataGridView.Rows[i].Cells["审批单价"].Value.ToString() == "" ||
                         ly_outmachine_contract_detail_selDataGridView.Rows[i].Cells["单价"].Value.ToString() == "")
                    {
                        MessageBox.Show("结账单明细中，存在审批单价与单价为空情况.", "注意");
                        return;
                    }
                    decimal p1 = decimal.Parse(ly_outmachine_contract_detail_selDataGridView.Rows[i].Cells["审批单价"].Value.ToString());
                    decimal p2 = decimal.Parse(ly_outmachine_contract_detail_selDataGridView.Rows[i].Cells["单价"].Value.ToString());
                    if (p1 - p2 > 0 || p1 - p2 < 0)
                    {
                        MessageBox.Show("结账单明细中，存在审批单价与单价不一致情况.", "注意");
                        return;
            ...
            //        else
            //        {
            //            dgv.CurrentRow.Cells["结算日期"].Value = "True";
            //            dgv.CurrentRow.Cells["结算日期"].Value = SQLDatabase.nowUserName();
        ...
        private void ly_cg_fk_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
