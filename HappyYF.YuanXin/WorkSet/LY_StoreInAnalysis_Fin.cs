...
{
    public partial class LY_StoreInAnalysis_Fin : Form
    {
         private DataSet ds;
        BindingSource bindingSource2;

        public LY_StoreInAnalysis_Fin()
        {
            InitializeComponent();
        }
        ...
        private void toolStripButton3_Click(object sender, EventArgs e)
        {
           ...
           // string sWhere = "  where ((a.service_number IS NULL) OR (a.service_number is not null)) ";
            ...
            if (this.checkBox8.Checked)
            {
                ...
                sWhere = sWhere + "  and ( a.input_date >= '" + this.dateTimePicker1.Value.Date.ToString () + "' and a.input_date < '" + this.dateTimePicker2.Value.Date.AddDays(1).ToString() + "') ";
            ...
            if (this.checkBox22.Checked)
            {
                ...
                ender = ender + " a.categoryCw,a.bill_code,";

                if (textBox17.Text.Replace(" ", "").Length > 0)
                {

                    sWhere = sWhere + " and  ( a.categoryCw = '" + textBox17.Text + "') ";
            ...
            if (this.checkBox21.Checked)
            {
                ...
                if (textBox16.Text.Replace(" ", "").Length > 0)
                {
                    ...
                    sWhere = sWhere + " and  ( a.category = '" + textBox16.Text + "') ";
            ...
            //////////////////////////////////////////////////
                ...
                {
                    header = header + "  month(a.input_date) as 月份, ";
                    ender = ender + "  month(a.input_date),";
            ...
            string aaaa = @" case when isnull(a.categoryCw,'')='原材料' and isnull(a.in_style,'未录')='采购入库'
                                    then  (case when sum( isnull(a.qty,0))<>0 then sum(isnull(a.materialB,0) * isnull(a.qty,0))/nullif(sum(isnull(a.qty,0)),null) else 0 end)
                                  when isnull(a.categoryCw,'')='原材料' and isnull(a.in_style,'未录')='外协采购'
                                    then  (case when sum( isnull(a.qty,0))<>0 then sum( (isnull(a.materialB,0)+isnull(a.outsourceB,0)) * isnull(a.qty,0))/nullif(sum(isnull(a.qty,0)),null) else 0 end)
                                  when isnull(a.categoryCw,'')='原材料' and isnull(a.in_style,'未录')='半品生产入库' and left(a.bill_code,2)<>'ZJ'
                                    then  (case when sum( isnull(a.qty,0))<>0 then sum(isnull(a.materialB,0) * isnull(a.qty,0))/nullif(sum(isnull(a.qty,0)),null) else 0 end)
                                  when isnull(a.categoryCw,'')='原材料' and isnull(a.in_style,'未录')='半品生产入库' and left(a.bill_code,2)='ZJ'
                                    then  (case when sum( isnull(a.qty,0))<>0 then sum(isnull(a.outsourceB,0) * isnull(a.qty,0))/nullif(sum(isnull(a.qty,0)),null) else 0 end)

                                  when isnull(a.categoryCw,'')='半成品' and isnull(a.in_style,'未录')='外协采购'
                                    then  (case when sum( isnull(a.qty,0))<>0 then sum(isnull(a.outsourceB,0) * isnull(a.qty,0))/nullif(sum(isnull(a.qty,0)),null) else 0 end)
                                   when isnull(a.categoryCw,'')='半成品' and ( isnull(a.in_style,'未录')='半品生产入库' OR  isnull(a.in_style,'未录')='采购入库' OR  isnull(a.in_style,'未录')='盘点')
                                    then  (case when sum( isnull(a.qty,0))<>0 then sum( (isnull(a.materialB,0)+isnull(a.outsourceB,0)+isnull(a.outsource_machineB,0))* isnull(a.qty,0))/nullif(sum(isnull(a.qty,0)),null) else 0 end)
                                 
                                   when isnull(a.categoryCw,'')='成品' 
                                    then  (case when sum( isnull(a.qty,0))<>0 then sum( (isnull(a.materialB,0)+isnull(a.outsourceB,0)+isnull(a.outsource_machineB,0)+isnull(a.labor_costB,0)+isnull(a.operation_costB,0) )* isnull(a.qty,0))/nullif(sum(isnull(a.qty,0)),null) else 0 end)
                                 
        
                             else sum(isnull(a.store_money,0))  end  入库单价 , ";


            string bbbbb = @"    case when isnull(a.categoryCw,'')='原材料' and isnull(a.in_style,'未录')='采购入库'
                                 then  (case when sum( isnull(a.qty,0))<>0 then sum(isnull(a.materialB,0) * isnull(a.qty,0))   else 0 end)
                               when isnull(a.categoryCw,'')='原材料' and isnull(a.in_style,'未录')='外协采购'
                                 then  (case when sum( isnull(a.qty,0))<>0 then sum( (isnull(a.materialB,0)+isnull(a.outsourceB,0)) * isnull(a.qty,0))  else 0 end)
                               when isnull(a.categoryCw,'')='原材料' and isnull(a.in_style,'未录')='半品生产入库' and left(a.bill_code,2)<>'ZJ'
                                    then  (case when sum( isnull(a.qty,0))<>0 then sum(isnull(a.materialB,0) * isnull(a.qty,0))  else 0 end)
                                  when isnull(a.categoryCw,'')='原材料' and isnull(a.in_style,'未录')='半品生产入库' and left(a.bill_code,2)='ZJ'
                                    then  (case when sum( isnull(a.qty,0))<>0 then sum(isnull(a.outsourceB,0) * isnull(a.qty,0))  else 0 end)
                               when isnull(a.categoryCw,'')='半成品' and isnull(a.in_style,'未录')='外协采购'
                                 then  (case when sum( isnull(a.qty,0))<>0 then sum(isnull(a.outsourceB,0) * isnull(a.qty,0))  else 0 end)
                                when isnull(a.categoryCw,'')='半成品' and ( isnull(a.in_style,'未录')='半品生产入库' OR  isnull(a.in_style,'未录')='采购入库' OR  isnull(a.in_style,'未录')='盘点')
                                 then  (case when sum( isnull(a.qty,0))<>0 then sum( (isnull(a.materialB,0)+isnull(a.outsourceB,0)+isnull(a.outsource_machineB,0))* isnull(a.qty,0))  else 0 end)
                              
                                when isnull(a.categoryCw,'')='成品' 
                                 then  (case when sum( isnull(a.qty,0))<>0 then sum( (isnull(a.materialB,0)+isnull(a.outsourceB,0)+isnull(a.outsource_machineB,0)+isnull(a.labor_costB,0)+isnull(a.operation_costB,0) )* isnull(a.qty,0))  else 0 end)
                              
                              
                               else sum(isnull(a.store_money,0))  end  入库金额 ,";




                string body = " sum( isnull(a.qty,0))  as 数量," +aaaa+ bbbbb +
            ...
            //" case when sum( isnull(a.qty,0))<>0 then sum(isnull(a.store_money,0))/nullif(sum(isnull(a.qty,0)),null) else 0 end as 库存单价,  " +
            ...
            //" sum(isnull(a.store_money,0)) as 入库金额," +
            ...
            " FROM ly_store_in_View_Fin AS a ";
            ...
            this.dataGridView1.Columns["数量"].DefaultCellStyle.Format ="N2";

            //this.dataGridView1.Columns["财务单价"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            //this.dataGridView1.Columns["财务单价"].DefaultCellStyle.Format = "N5";

            //this.dataGridView1.Columns["采购单价"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            //this.dataGridView1.Columns["采购单价"].DefaultCellStyle.Format = "N5";

            //this.dataGridView1.Columns["库存单价"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            //this.dataGridView1.Columns["库存单价"].DefaultCellStyle.Format = "N5";

            this.dataGridView1.Columns["入库金额"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            this.dataGridView1.Columns["入库金额"].DefaultCellStyle.Format = "N5";
        ...
        private void AddSummationRow(BindingSource bs, DataGridView dg)
        {
            ...
            foreach (DataGridViewRow dr in dg.Rows)
            {
                ...
                if (System.DBNull.Value == dr.Cells["入库金额"].Value)
                    ...
                    sum_storemoney = sum_storemoney + decimal.Parse(dr.Cells["入库金额"].Value.ToString());
            ...
            dg.CurrentRow.Cells["入库金额"].Value = sum_storemoney;
        ...
        {
            string sel = "SELECT distinct  category as 组别 FROM ly_store_in_View_Fin";
