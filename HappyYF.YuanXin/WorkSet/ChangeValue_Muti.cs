...
{
    public partial class ChangeValue_Muti : Form
...

    {
        ...
        {
            get { return newValue2; }
            set { newValue2 = value; }
        }

        private string newValue3;

        public string NewValue3
        {
            get { return newValue3; }
            set { newValue3 = value; }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (this.changeMode != "datetime")
            {
                if (this.changeMode != "string" && this.changeMode != "longstring")
                {
                    ...
                    //if (!(Regex.IsMatch(this.textBox3.Text, @"^([-]{1}[0-9]+(.[0-9]{2})?$)|([0-9]+(.[0-9]{2})?$)"))) // ^\+?[1-9][0-9]*$
                    //{
                    //    MessageBox.Show("数字格式错误，请重新输入");
                    //    this.textBox3.Focus();
                    //    return;
                ...
                NewValue2 = this.textBox3.Text;
                NewValue3 = this.textBox4.Text;
            }
            else
            {
