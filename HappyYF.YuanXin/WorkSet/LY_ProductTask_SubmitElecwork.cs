...
{
    public partial class LY_ProductTask_SubmitElecwork : Form
    {
        public LY_ProductTask_SubmitElecwork()
        {
            InitializeComponent();
        }
        ...
        private void LY_QualityInspection_Benchwork_Load(object sender, EventArgs e)
        {
            ...
            this.ly_product_task_requestSingleTableAdapter.Connection.ConnectionString = SQLDatabase.Connectstring;

            this.ly_product_task_returnSingleTableAdapter.Connection.ConnectionString = SQLDatabase.Connectstring;
          
            this.ly_product_returnTableAdapter.Connection.ConnectionString = SQLDatabase.Connectstring;
            this.ly_product_wasteSelectTableAdapter.Connection.ConnectionString = SQLDatabase.Connectstring; 
            this.ly_product_task_wasteTableAdapter.Connection.ConnectionString = SQLDatabase.Connectstring;
        ...
        protected void reLoad()
        {
            ...
            //this.ly_product_returnTableAdapter.Fill(this.lYProductMange.ly_product_return, int.Parse(plan_id), int.Parse(tjh));//可追加退料  选择列表

            this.ly_product_returnTableAdapter.Fill(this.lYProductMange.ly_product_return, int.Parse(plan_id));
            this.ly_product_wasteSelectTableAdapter.Fill(this.lYProductMange.ly_product_wasteSelect, int.Parse(plan_id));
            //this.ly_product_wasteSelectTableAdapter.Fill(this.lYProductMange.ly_product_wasteSelect, int.Parse(plan_id), int.Parse(tjh));
        ...
        private void ly_material_replacelistDataGridView_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(SQLDatabase.Connectstring))
            {
                string sql = "select material_flag,request_qty from ly_production_task_requestSingle where task_inspection=" + task_inspection + " and itemno='" + wzbh + "' and origin_itemno='" + old_wzbh + "'";
        ...
        private void ly_Restructuring_request_singleDataGridView_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if ("申请说明" == dgv.CurrentCell.OwningColumn.Name)
            {
                ...
                if (queryForm.NewValue != "")
                {
                    using (SqlConnection connection = new SqlConnection(SQLDatabase.Connectstring))
                    {
                        dgv.CurrentRow.Cells["申请说明"].Value = queryForm.NewValue;
        ...
        private void ly_Restructuring_return_singleDataGridView_CellMouseDoubleClick_1(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (null == this.ly_Restructuring_return_singleDataGridView.CurrentRow) return;
            ...
            if ("申请说明2" == dgv.CurrentCell.OwningColumn.Name)
            {
                ...
                if (queryForm.NewValue != "")
                {
                    using (SqlConnection connection = new SqlConnection(SQLDatabase.Connectstring))
                    {
                        dgv.CurrentRow.Cells["申请说明2"].Value = queryForm.NewValue;
