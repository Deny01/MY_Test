...
{
    public partial class LY_MaterialBom : Form
    {

        string parentNum = "noSet";
        
        public LY_MaterialBom()
        {
            InitializeComponent();
        }
            ...
            try
            {
                if (dr.Length > 0)
                {
                    foreach (DataRow d in dr)
                    {
                        ...
                        {
                            if (string.IsNullOrEmpty(d["absqty"].ToString()))
                            {
                                TNode.ToolTipText = d["parentno"].ToString() + "0";
                            }
                            else
                            {
        ...
        private void toolStripTextBox2_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            dFilter = GetDataGridviewMultiFilterString.DGVMultiFilterString(this.bom_material_selDataGridView, this.toolStripTextBox2.Text);
        ...
        private void replaceitem(string oriitemno, string disitemno)
        {
            SqlConnection sqlConnection1 = new SqlConnection(SQLDatabase.Connectstring);
            SqlCommand cmd = new SqlCommand();
            ...
            cmd.CommandText = "LY_Bom_ReplaceItem";
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Connection = sqlConnection1;
        ...
        private void 物料批量替换ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (null == ly_inma0010DataGridView.CurrentRow) return;
            if (!checkLock())
            {
            ...
            if (SQLDatabase.CheckHaveRight(SQLDatabase.NowUserID, "物料批量替换"))
            {

            }
            else
            {
                MessageBox.Show("无(BOM)物料替换权限", "注意");
                return;
            }
            ...
            string message1 = "用新物料:" + queryForm.Result + " 替换BOM中的所有编码为：" + nowitemno + "，继续吗？";
            string caption1 = "提示...";
            MessageBoxButtons buttons1 = MessageBoxButtons.YesNo;
            ...
            if (result1 == DialogResult.Yes)
            {
                ...
                replaceitem(nowitemno,queryForm.Result);
            }
        ...
        private void deleteitem(string oriitemno)
        {
            SqlConnection sqlConnection1 = new SqlConnection(SQLDatabase.Connectstring);
            SqlCommand cmd = new SqlCommand();
           ...
           // string contractinnerCode = "";
            ...
            cmd.CommandText = "LY_Bom_DeleteItem";
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Connection = sqlConnection1;
        ...
        private void 物料批量删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (null == ly_inma0010DataGridView.CurrentRow) return;
            if (!checkLock())
            {
            ...
            if (SQLDatabase.CheckHaveRight(SQLDatabase.NowUserID, "物料批量删除"))
            {

            }
            else
            {
                MessageBox.Show("无(BOM)物料删除权限", "注意");
                return;
            }
            ...
            //    return;
            //}
            string message1 = "删除BOM中的所有编码为：" + nowitemno + "的物料，继续吗？";
            string caption1 = "提示...";
            MessageBoxButtons buttons1 = MessageBoxButtons.YesNo;
            ...
            if (result1 == DialogResult.Yes)
            {
              ...
              deleteitem(nowitemno);
            }

            MessageBox.Show("删除完成,请检查替换结果...", "注意");
        }
