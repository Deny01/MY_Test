...
{
    public partial class LY_Quality_Control_PurchaseREC : Form
    {

        string nowcontractNum;

        public string NowcontractNum
        {
            get { return nowcontractNum; }
            set { nowcontractNum = value; }
        }

        DateTime nowsignDate;

        public DateTime NowsignDate
        {
            get { return nowsignDate; }
            set { nowsignDate = value; }
        }
       ...
       // string now_order_id = "";
        ...
        public LY_Quality_Control_PurchaseREC()
        {
            InitializeComponent();
        }
        ...
        private void LY_Machine_Load(object sender, EventArgs e)
        {
            ...
            this.dateTimePicker1.Text = nowsignDate.AddDays(-9).Date.ToString();
            this.dateTimePicker2.Text = DateTime.Today.AddDays(9).Date.ToString();
            //////////////////////////////////////////////////////////////////////////////////////////
            ...
            {
                this.ly_purchase_contract_mainQClistBindingSource.Filter = "采购员='" + SQLDatabase.nowUserName() + "'";
            }
            ...
            this .ly_purchase_contract_mainQClistBindingSource.Position = this .ly_purchase_contract_mainQClistBindingSource.Find ("合同编号",nowcontractNum);
        ...
        private bool HaveCheck()
        {
            decimal checknum=0;

            if (!string.IsNullOrEmpty(this.ly_purchase_contract_inspectionDataGridView.CurrentRow.Cells["入库数量8"].Value.ToString()))
            {
                checknum = checknum + decimal.Parse(this.ly_purchase_contract_inspectionDataGridView.CurrentRow.Cells["入库数量8"].Value.ToString());
            }

            if (!string.IsNullOrEmpty(this.ly_purchase_contract_inspectionDataGridView.CurrentRow.Cells["合格数量8"].Value.ToString()))
            {
                checknum = checknum + decimal.Parse(this.ly_purchase_contract_inspectionDataGridView.CurrentRow.Cells["合格数量8"].Value.ToString());
            }

            if (!string.IsNullOrEmpty(this.ly_purchase_contract_inspectionDataGridView.CurrentRow.Cells["可用数量8"].Value.ToString()))
            {
                checknum = checknum + decimal.Parse(this.ly_purchase_contract_inspectionDataGridView.CurrentRow.Cells["可用数量8"].Value.ToString());
            }

            if (0 < checknum    )
            {
                MessageBox.Show("已有检验结果,操作取消", "注意");
                return true;
        ...
        private void SaveChanged()
        {
           ...
           //// this.ly_production_order_inspectionAllTableAdapter.Fill(this.lYQualityInspector.ly_production_order_inspectionAll, nowproductionorderAllNum);
            ...
            string componentNum="";
            if (null != ly_purchase_contract_inspectionDataGridView.CurrentRow)
            {
                 componentNum = ly_purchase_contract_inspectionDataGridView.CurrentRow.Cells["物料编号8"].Value.ToString();
            }
        ...
        private void ly_production_order_inspectionDataGridView_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            string componentNum = dgv.CurrentRow.Cells["物料编号8"].Value.ToString();
            ...
            if ("收料数量8" == dgv.CurrentCell.OwningColumn.Name)
            {
                ...
                decimal  nowInspectQty_Int;

                if (string.IsNullOrEmpty(nowInspectQty))
                {
                    nowInspectQty_Int = 0;
                }
                else
                {
                    ...
                    nowInspectQty_Int = decimal.Parse(nowInspectQty);
                }


                if (queryForm.NewValue != "")
                {
                    dgv.CurrentRow.Cells["收料数量8"].Value = queryForm.NewValue;
                    ...
                    else
                    {
                        ...
                        old_value = decimal.Parse(queryForm.OldValue);
                    }
                    ...
                    else
                    {
                        ...
                        new_value = decimal.Parse(queryForm.NewValue);
                    }

                    //if (new_value > old_value)
                    if (new_value > nowInspectQty_Int)
                    {
                        dgv.CurrentRow.Cells["收料数量8"].Value = queryForm.OldValue;
                        MessageBox.Show("修改数量不能大于原来数量", "注意");
                        return;
                    }
                


                    //if ((new_value - old_value) > nowInspectQty_Int)
                    //{
                    //    MessageBox.Show("送检总数不能大于合同采购数", "注意");
                    //    dgv.CurrentRow.Cells["收料数量8"].Value = queryForm.OldValue;
                    ...
                    if (!CheckInput(dgv))
                    {
                        if (string.IsNullOrEmpty(queryForm.OldValue))
                        {
                            ...
                            dgv.CurrentRow.Cells["收料数量8"].Value = DBNull.Value;
                        }
                        else
                        {
                            //dgv.CurrentRow.Cells["送检数量8"].Value = queryForm.OldValue;
...
//            }
        ...
        private void toolStripTextBox2_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            {
                this.ly_purchase_contract_mainQClistBindingSource.Filter = "(" + dFilter + ") and (采购员='" + SQLDatabase.nowUserName() + ")'";
            }
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            ...
            this.nowdate = SQLDatabase.GetNowdate().ToString("yyyy-MM-dd");
        ...
        private void toolStripButton5_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(nowoperator))
            {
            ...
            if (ly_purchase_contract_inspectionDataGridView.Rows.Count > 0)
            {
                for (int i = 0; i < ly_purchase_contract_inspectionDataGridView.Rows.Count; i++)
                {

                    if (ly_purchase_contract_inspectionDataGridView.Rows[i].Cells["入库数量8"].Value.ToString().Trim() != "0.00000")
                    {
        ...
        private void toolStripButton6_Click(object sender, EventArgs e)
        {
            ...
            string nowoperator = this.ly_purchase_contract_inspection_MainDataGridView.CurrentRow.Cells["质检员"].Value.ToString().TrimEnd();
            if (!string.IsNullOrEmpty(nowoperator))
            {
                if (nowoperator != SQLDatabase.nowUserName())
                {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                string delstr = " delete ly_purchase_contract_inspection  from ly_purchase_contract_inspection  " +
                    " where isnull(quality_inspector,'')='' and contract_inspection_code = '" + InspectionCode + "'";
        ...
        private void ly_purchase_contract_detailDataGridView_DoubleClick(object sender, EventArgs e)
        {
           ...
           "(             contract_code,     contract_inspection_code,       itemno,         receive_count,        inspect_date,id2,part_name,instore_flag) " +
           " values ('" + nowcontractcode + "','" + InspectionCode + "','" + componentNum + "'," + nowInspectQty + ",'"  + nowdate + "',"  + nowid2 + ",'" + nowpartname + "'," + nowinstoreflag + " )";
        ...
        private void SaveIndate(string indate)
        {
            ...
            string delstr = " update ly_purchase_contract_inspection  set inspect_date='" + indate + "'" +
                " from ly_purchase_contract_inspection " +
               " where ly_purchase_contract_inspection.contract_inspection_code = '" + inspectnumber + "'";
        ...
        private void 打印PToolStripButton_Click(object sender, EventArgs e)
        {
            ...
            if (null == ly_purchase_contract_inspectionDataGridView.CurrentRow) return;
            ...
            queryForm.Text = "中原精密采购收料送检单";

            queryForm.Printdata = this.lYQualityInspector;

            queryForm.PrintCrystalReport = new LY_Purchase_rec();
