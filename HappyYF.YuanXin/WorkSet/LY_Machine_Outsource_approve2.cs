...
namespace HappyYF.YuanXin.WorkSet
{
    public partial class LY_Machine_Outsource_approve2 : Form
    {
        ...
        private void LY_Machine_Outsource_Load(object sender, EventArgs e)
        {
            ...
            this.lY_outmachine_contract_main_selBindingSource.Filter = "批准  = 'True'";
        ...
        private void lY_outmachine_contract_main_selDataGridView_SelectionChanged(object sender, EventArgs e)
        {
            ...
            if (null != this.lY_outmachine_contract_main_selDataGridView.CurrentRow)
            {
                ...
                ly_PrepaymentTableAdapter.Fill(lYMaterielRequirements.ly_Prepayment, nowcontract);
            }
            else
            {
        ...
        private void lY_outmachine_contract_main_selDataGridView_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            {
                MessageBox.Show("合同已经付款,不能修改...", "注意");
                return;
            ...
            if (string.IsNullOrEmpty(dgv.CurrentRow.Cells["合同编号"].Value.ToString()))
            {
                MessageBox.Show("合同编号为空...", "注意");
                return;

            }
      
            string sql = @"select isnull(approve,0) from ly_outmachine_contract_main   where contract_code = '"+ dgv.CurrentRow.Cells["合同编号"].Value.ToString() + "' ";
