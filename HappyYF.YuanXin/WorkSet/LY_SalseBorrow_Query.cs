...
{
    public partial class LY_SalseBorrow_Query : Form
    {
        ...
        private void Yonghu_Load(object sender, EventArgs e)
        {
           ...
           // this.ly_sales_clientTableAdapter.Fill(this.lYSalseMange.ly_sales_client);
            ...
            this.ly_sales_borrow_detail_allBindingSource.Filter = "仓库未还>0";
            this.ly_sales_borrow_detail_sumBindingSource.Filter = "仓库未还>0";
        ...
        {
            if ("显示全部" == toolStripButton17.Text)
            {
                toolStripButton17.Text = "显示未还";
                this.ly_sales_borrow_detail_allBindingSource.Filter = "仓库未还>0";
            }
            else
            {
                toolStripButton17.Text = "显示全部";
                this.ly_sales_borrow_detail_allBindingSource.Filter = "";
            }
        }
        ...
        {
            if ("显示全部" == toolStripButton18.Text)
            {
                toolStripButton18.Text = "显示未还";
                this.ly_sales_borrow_detail_sumBindingSource.Filter = "仓库未还>0";
            }
            else
            {
                toolStripButton18.Text = "显示全部";
                this.ly_sales_borrow_detail_sumBindingSource.Filter = "";
            }
        }

        private void ly_sales_borrow_detail_allDataGridView_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            DateTime nowdate = SQLDatabase.GetNowdate();

            foreach (DataGridViewRow dgr in ly_sales_borrow_detail_allDataGridView.Rows)
            {
                if (decimal.Parse(dgr.Cells["仓库未还"].Value.ToString()) <= 0)
                {

                    continue;  
                }

                if ( (DateTime.Parse(dgr.Cells["借用日期"].Value.ToString()).AddMonths(3) <= nowdate) && ((DateTime.Parse(dgr.Cells["返还日期"].Value.ToString()) <= this.dateTimePicker2.Value)))
                {
        ...
        private void ly_sales_clientDataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
