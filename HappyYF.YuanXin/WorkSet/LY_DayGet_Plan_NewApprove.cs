...
{
    public partial class LY_DayGet_Plan_NewApprove : Form
    {
         string formState = "View";

         public LY_DayGet_Plan_NewApprove()
        {
            InitializeComponent();
        }
        ...
        private void LY_Material_Plan_Load(object sender, EventArgs e)
        {
            ...
            if ("0003" == SQLDatabase.nowUserDepartmentBig() || "000" == SQLDatabase.NowUserID || "999" == SQLDatabase.NowUserID || "998" == SQLDatabase.NowUserID || "66" == SQLDatabase.NowUserID)
            {
                ...
                //清单编码Label.Visible = true;
                this.adjust_salseBillCodeTextBox.Visible = true;
            ...
            else
            {
                ...
                //清单编码Label.Visible = false;
                this.adjust_salseBillCodeTextBox.Visible = false;
        ...
        private string GetMaxPlanCode()
        {
            ...
            cmd.Parameters["@Plan_mode"].Value = "TZJH";
        ...
        private void bindingNavigatorDeleteItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("调整计划已经审批,不能删除...", "注意");
                return;
            }
        ...
        private void ly_inma0010_planselDataGridView_DoubleClick(object sender, EventArgs e)
        {
            ...
            if ("True" == this.ly_material_plan_mainDataGridView.CurrentRow.Cells["启用"].Value.ToString())
            {
                MessageBox.Show("库存调整已经审批,不能修改条目...", "注意");
                return;
            }
            if ("True" == this.ly_material_plan_mainDataGridView.CurrentRow.Cells["检验"].Value.ToString())
            {
                MessageBox.Show("库存调整已经审批,不能修改条目...", "注意");
                return;
            }
            string salespeople = this.ly_material_plan_mainDataGridView.CurrentRow.Cells["制定人"].Value.ToString();
        ...
        private void ly_material_plan_mainDataGridView_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if ("检验" == dgv.CurrentCell.OwningColumn.Name)
            {
                if (!SQLDatabase.CheckHaveRight(SQLDatabase.NowUserID, "库存调整检验"))
                {
                    MessageBox.Show("无检验权限...", "注意");
                ...
                {
                    MessageBox.Show("已部门批准...", "注意");
                    return;

                }

                if ("True" == dgv.CurrentRow.Cells["检验"].Value.ToString())
                {
                    dgv.CurrentRow.Cells["检验"].Value = "False";

                    dgv.CurrentRow.Cells["检验人"].Value = DBNull.Value;
                    dgv.CurrentRow.Cells["检验日期"].Value = DBNull.Value;
                }
                else
                {

                    dgv.CurrentRow.Cells["检验"].Value = "True";

                    dgv.CurrentRow.Cells["检验人"].Value = SQLDatabase.nowUserName();
                    dgv.CurrentRow.Cells["检验日期"].Value = SQLDatabase.GetNowdate();
            ...
            {
                if (!SQLDatabase.CheckHaveRight(SQLDatabase.NowUserID, "库存调整部门批准"))
                {
                    MessageBox.Show("无部门批准权限...", "注意");
            ...
            {
                if (!SQLDatabase.CheckHaveRight(SQLDatabase.NowUserID, "库存调整审批"))
                {
                    MessageBox.Show("无库存调整审批权限...", "注意");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (string .IsNullOrEmpty(this.客户编码TextBox.Text))
            {
                MessageBox.Show("请先选择客户...", "注意");
                return;
            }

            string sel;
           
                sel = "SELECT  a.contract_inner_code as 清单,a.contract_code as 合同,b.salesclient_code as 客户代码,b.salesclient_name as 客户 FROM ly_sales_contract_main a left join ly_sales_client_forcontract AS b ON a.contract_code = b.contract_inner_code  where b.salesclient_code='"+ this.客户编码TextBox.Text + "' order by a.contract_inner_code desc ";
            ...
            this.adjust_salseBillCodeTextBox.Text = queryForm.Result;
            //this.客户编码TextBox.Text = queryForm.Result1;
            //this.营业员码TextBox.Text = queryForm.Result2;
            //this.营业员TextBox.Text = queryForm.Result3;
        }
