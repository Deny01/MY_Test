...
{
    public partial class LY_Restructuring_Plan : Form
    {
         string formState = "View";

         public LY_Restructuring_Plan()
        {
            InitializeComponent();
        }
        ...
        private void bindingNavigatorAddNewItem_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                this.ly_material_plan_mainDataGridView.CurrentRow.Cells["录入日期"].Value = SQLDatabase.GetNowdate();
        ...
        private void ly_inma0010_planselDataGridView_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(this.lY_dayget_material_selDataGridView.CurrentRow.Cells["工时"].Value.ToString()) ||
                string.IsNullOrEmpty(this.lY_dayget_material_selDataGridView.CurrentRow.Cells["工价"].Value.ToString())  )
            {
                MessageBox.Show("请联系技术部添加改制工时和工价...", "注意");
                return;
            }
            ...
            string componentNum = this.lY_dayget_material_selDataGridView.CurrentRow.Cells["改后编号_N"].Value.ToString();
            string componentNum_Old = this.lY_dayget_material_selDataGridView.CurrentRow.Cells["改前编号_N"].Value.ToString();

            int parentId = int.Parse(this.ly_material_plan_mainDataGridView.CurrentRow.Cells["id"].Value.ToString());
            string prod_dept = SQLDatabase.nowUserDepartmentBig();

            string remark = this.lY_dayget_material_selDataGridView.CurrentRow.Cells["备注2"].Value.ToString();


            InserPlanDetail(componentNum, parentId, 0, componentNum_Old,remark); 
        ...
        private static void InserPlanDetail(string componentNum, int parentId, decimal nowabsqty,string componentNum_Old,string remark)
        {
            string insStr = " INSERT INTO ly_material_plan_detail  " +
           "( plan_id,wzbh,plan_count,origin_itemno,remark) " +
           " values ('" + parentId + "','" + componentNum + "'," + nowabsqty + ",'"+ componentNum_Old + "','"+remark+"')";
