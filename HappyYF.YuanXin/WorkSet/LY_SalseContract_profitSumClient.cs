...
{
    public partial class LY_SalseContract_profitSumClient : Form
    {
        public LY_SalseContract_profitSumClient()
        {
            InitializeComponent();
        }
        ...
        private void LY_sales_ClientSUM_Marposs_Load(object sender, EventArgs e)
        {
            ...
            this.ly_sales_profit_sumClientTableAdapter.Connection.ConnectionString = SQLDatabase.Connectstring;
        }

        private void toolStripButton38_Click(object sender, EventArgs e)
        {
            this.ly_sales_profit_sumClientTableAdapter.Fill(this.lYSalseMange2.ly_sales_profit_sumClient, this.dateTimePicker9.Value, this.dateTimePicker10.Value.AddDays(1));
        ...
        private void ly_sales_profit_sumDataGridView_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if ("业务费用因子" == dgv.CurrentCell.OwningColumn.Name || "利润率基值" == dgv.CurrentCell.OwningColumn.Name)
            {
                ...
                if (queryForm.NewValue != "")
                {
                    ...
                    {
                        updstr = " update ly_sales_profit_arrt  set buseness_arrt_client = " + queryForm.NewValue;
                    }
                    else
                    {
                        updstr = " update ly_sales_profit_arrt  set profit_arrt_client = " + queryForm.NewValue;
